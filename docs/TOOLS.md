...
Provision a PKCS#8 Ed25519 key through `EVALS_DATASET_SIGNING_KEY_FILE`. With `EVALS_MIGRATION_DATABASE_URL_FILE` pointing at the migration-owner connection, run `npm run evals:release-check-stage-e`. The checker is read-only and passes only when migrations 043–044 exist, `evals_runtime` is least-privileged and owns no evaluation objects, required immutable triggers are enabled, the signing key is Ed25519 and `EVALS_EXPERT_WORK_ENABLED=true`. Its output contains identifiers/counts only. Keep key material out of commands, logs and documentation.
