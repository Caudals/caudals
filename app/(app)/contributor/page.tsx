      ...
      <div className="flex flex-1 flex-col gap-6 p-6">
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Card className="border-emerald-500/20">
            <CardHeader>
              ...
              <CardDescription>{t("Track your earnings and payouts")}</CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-muted-foreground">{t("Total Earned")}</span>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t("Browse available datasets and contribute")}</CardDescription>
            </CardHeader>
            <CardContent className="space-y-2">
              <Button asChild className="w-full">
                <Link href="/browse" data-dashboard-action="contributor_browse_datasets">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                {t("Improve profile completeness to increase requester confidence")}
              </CardDescription>
            </CardHeader>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>{t("Track your contributions across stages")}</CardDescription>
          </CardHeader>
          <CardContent>
            <div className="grid gap-4 md:grid-cols-3">
              <div className="rounded-lg border bg-blue-500/5 p-4">
                ...
                <p className="mt-1 text-xs text-muted-foreground">{t("All contributions")}</p>
              </div>
              ...
              <div className="rounded-lg border bg-yellow-500/5 p-4">
                ...
                <p className="mt-1 text-xs text-muted-foreground">{t("Awaiting approval")}</p>
              </div>
              ...
              <div className="rounded-lg border bg-emerald-500/5 p-4">
                ...
                <p className="mt-1 text-xs text-muted-foreground">{t("Earning rewards")}</p>
              </div>
            </div>
          </CardContent>
        </Card>
        ...
        <div className="grid gap-4 xl:grid-cols-3">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>{t("Expected and blocked payout amounts")}</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              <ForecastRow
                label={t("Ready to payout")}
                value={`$${(essentialsData?.payoutForecast.readyAmount ?? 0).toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}`}
              ...
              <ForecastRow
                ...
                value={`$${(essentialsData?.payoutForecast.pendingTransferAmount ?? 0).toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}`}
              ...
              <ForecastRow
                ...
                value={`$${(essentialsData?.payoutForecast.failedTransferAmount ?? 0).toLocaleString(undefined, {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}`}
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              {t("Submissions requiring revision or containing reviewer feedback")}
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-3">
                ...
                >
                  <div className="space-y-1">
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {item.status === "rejected" ? t("Rejected") : t("Needs changes")}
                      </Badge>
                    </div>
                    <p className="text-xs text-muted-foreground">
                      {t("Last update")}: {new Date(item.updatedAt).toLocaleDateString()}
                    </p>
                    <p className="text-xs text-muted-foreground line-clamp-2">
  ...
  return (
    <div className="rounded-xl border border-border/70 p-3">
      <div className="flex items-center justify-between">
        ...
        <p className={tone === "danger" ? "text-sm font-semibold text-destructive" : "text-sm font-semibold"}>
          {value}
        </p>
      </div>
