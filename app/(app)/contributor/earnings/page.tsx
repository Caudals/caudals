...
export default async function EarningsPage() {
  ...
  return (
    <>
      <ContributorPageHeader title="Earnings & Payouts" />
      <div className="flex flex-1 flex-col gap-6 p-6">
        <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          ...
          <Card>
            ...
            <CardContent>
              <div className="text-xs text-muted-foreground">Before platform fee</div>
            </CardContent>
          </Card>
        </div>
              ...
              <div className="space-y-3">
                ...
                <ul className="space-y-2 text-sm text-muted-foreground">
                  <li className="flex items-start gap-2">
                    ...
                    <span>Approved: your submission is approved by review team.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <Clock className="mt-0.5 h-4 w-4 text-amber-600" />
                    <span>Transfer created: payout transaction enters pending queue.</span>
                  </li>
                  <li className="flex items-start gap-2">
                    <CheckCircle className="mt-0.5 h-4 w-4 text-emerald-600" />
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>Your recent payouts and transaction status timeline</CardDescription>
          </CardHeader>
          <CardContent>
            {payouts.length > 0 ? (
              <div className="space-y-3">
                {payouts.map((payout) => {
                  const transferCreated = Boolean(payout.reference_id) || payout.status !== "cancelled";
                    ...
                    >
                      <div className="flex flex-wrap items-center justify-between gap-4">
                        <div className="flex flex-1 items-center gap-3">
                          ...
                          <div>
                            <p className="font-medium">{payout.description || "Payout"}</p>
...
function TimelinePill({
  ...
  return (
    <div className={`rounded-md border px-3 py-2 text-xs font-medium ${classes}`}>
      {label}
    </div>
  );
}
