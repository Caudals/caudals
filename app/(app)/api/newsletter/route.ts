 ...
 * record, the consent trail and the welcome email belong to the leads project's
 * `newsletter-subscribe` edge function, which is the only thing allowed to write
 * to the list.
 *
 * Signup is single opt-in: the address is confirmed on submit and the email is a
 * receipt, not a gate.
 */
...
export async function POST(request: NextRequest) {
  ...
  if (
    ...
    return withHeaders(
      NextResponse.json({ success: true, newsletter: "confirmed" }),
      ipRateHeaders
    );
  }
