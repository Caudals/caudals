...
import { readExpertAssignment, revealReviewPhase } from "@/lib/evals/experts/store";
...
export const PATCH = api(async (request, identity) => {
  requireStageEEnabled(); if (!identity.platformRole) throw new EvalError("SCOPE_DENIED", 404);
  const input = z.strictObject({ orgId: z.uuid(), action: z.literal("reveal_review") }).parse(await jsonBody(request));
  await requireWorkspace(identity, input.orgId, "manage");
  return revealReviewPhase({ orgId: input.orgId, actorId: identity.user.id }, id(request));
});
