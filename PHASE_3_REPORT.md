...
- Added a VPS-side platform completion gate, `npm run platform:completion-status`, that aggregates the final production checks for landing-mode routing, exact public nav labels, Sentry, operator MFA, private observability readiness, external alert routing, and the current-quarter pentest tracker.
