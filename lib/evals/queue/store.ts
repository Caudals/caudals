...
export async function enqueueBrowserDiscovery(client:PoolClient,tenant:Tenant,args:{workflowId:string;targetRevisionId:string;planHash:string;version:number;input:BrowserDiscovery}){const input=browserDiscoverySchema.parse(args.input),hash=digest(input);await client.query(`INSERT INTO evals.execution_workflow(id,org_id,run_id,plan_hash,created_by) VALUES($1,$2,$3,$4,$5) ON CONFLICT(org_id,id) DO NOTHING`,[args.workflowId,tenant.orgId,args.targetRevisionId,args.planHash,tenant.actorId]);const result=await client.query(`INSERT INTO evals.workflow_step(org_id,workflow_id,step_kind,input_hash,version,input) VALUES($1,$2,'execute_browser',$3,$4,$5) ON CONFLICT(org_id,workflow_id,step_kind,input_hash,version) DO NOTHING RETURNING id`,[tenant.orgId,args.workflowId,hash,args.version,input]);const id=result.rows[0]?.id??(await client.query("SELECT id FROM evals.workflow_step WHERE org_id=$1 AND workflow_id=$2 AND step_kind='execute_browser' AND input_hash=$3 AND version=$4",[tenant.orgId,args.workflowId,hash,args.version])).rows[0].id;if(result.rows[0]){await client.query("INSERT INTO evals.outbox_event(org_id,step_id,queue) VALUES($1,$2,'execute_browser')",[tenant.orgId,id]);await event(client,tenant.orgId,args.workflowId,'website_discovery_queued');}return id as string;}
