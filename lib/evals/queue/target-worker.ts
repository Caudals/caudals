...
type Step={id:string;workflow_id:string;input:unknown;input_hash:string;status:string;fence:string;lease_until:Date|null};
...
export class TargetExecutionWorker{
 ...
 async handle(raw:JobData){
  ...
  const claimed=await this.options.tx(tenant,async c=>{
   ...
   const attemptId=randomUUID(),target=(await c.query("SELECT document FROM evals.target_revision WHERE org_id=$1 AND id=$2",[tenant.orgId,input.targetRevisionId])).rows[0];
   ...
   return {step:updated,workflow,input,attemptId,config:targetConfigSchema.parse(target.document)};
  });
  if(!claimed)return;
  ...
  timer.unref();
  try{
   const dispatch=await this.options.tx(tenant,async c=>{const current=await locked(c,tenant.orgId,claimed.step.id);if(!["queued","running"].includes(current.workflow.status)){await c.query("UPDATE evals.target_attempt SET status='canceled',reason_code='workflow_not_dispatchable',finished_at=now() WHERE org_id=$1 AND id=$2",[tenant.orgId,claimed.attemptId]);await c.query("UPDATE evals.workflow_step SET status=$3,lease_until=NULL WHERE org_id=$1 AND id=$2",[tenant.orgId,claimed.step.id,current.workflow.status.startsWith("cancel")?"canceled":"paused"]);await c.query("UPDATE evals.case_unit SET status=$3,reason_code='workflow_not_dispatchable',updated_at=now() WHERE org_id=$1 AND id=$2",[tenant.orgId,claimed.input.caseUnitId,current.workflow.status.startsWith("cancel")?"canceled":"pending"]);await projectWorkflow(c,tenant.orgId,claimed.step.workflow_id);return false;}await c.query("UPDATE evals.target_attempt SET status='dispatching' WHERE org_id=$1 AND id=$2 AND status='claimed'",[tenant.orgId,claimed.attemptId]);await event(c,tenant.orgId,claimed.step.workflow_id,"target_dispatching");return true;});
   if(!dispatch)return;
