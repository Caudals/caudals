...
export const contextProfileSchema=z.strictObject({
  ...
  tool_descriptions:z.array(z.string()), unanswered_questions:z.array(z.strictObject({field:z.string(),question:z.string(),critical:z.boolean()})),
...
export function buildContextProfile(input:{evaluationId:string;purpose?:string;intendedUsers?:string[];tasks?:string[];languages?:string[];jurisdiction?:string;asOf?:string|null;sources:Array<{revisionId:string;authority?:string;applicableFrom?:string|null;applicableTo?:string|null;claims?:Record<string,string>}>;capabilities?:string[];risks?:string[];allowedActions?:string[];tools?:string[];promptRevision:string;modelRevisionId?:string|null}):ContextProfile {
  ...
  if(!input.languages?.length)unanswered.push({field:"languages",question:"Which languages must the system support?",critical:true});
  ...
  const document=withContentHash({schema_version:"1.0" as const,evaluation_id:input.evaluationId,purpose:{value:input.purpose??null,confidence:input.purpose?1:0,basis:[]},intended_users:input.intendedUsers??[],tasks:input.tasks??[],business_boundaries:[],supported_capabilities:input.capabilities??["text"],source_hierarchy:input.sources.map((source,index)=>({source_revision_id:source.revisionId,rank:index+1,authority:source.authority??"customer_supplied"})),languages:input.languages??[],jurisdiction:{value:input.jurisdiction??null,confidence:input.jurisdiction?1:0,basis:[]},as_of:input.asOf??null,material_risks:input.risks??[],allowed_actions:input.allowedActions??[],tool_descriptions:input.tools??[],unanswered_questions:unanswered,conflicts,prompt_revision:input.promptRevision,model_revision_id:input.modelRevisionId??null});
