...
export function genericGroundedQaPack(input:{source:Source;questions:Array<{question:string;expected:string;anchor:string;severity?:"low"|"medium"|"high"|"critical"}>;authorId:string}):{rubric:Rubric;cases:CefCase[]} {
  const validAnchors = new Set(input.source.anchors.map((anchor) => anchor.id));
  for (const question of input.questions) {
    if (!validAnchors.has(question.anchor)) throw new Error("Source anchor is absent from the frozen revision.");
  }
  ...
  const cases=input.questions.map((item,index)=>{const caseId=randomUUID(),revisionId=randomUUID();return withContentHash({schema_version:"1.0" as const,case_id:caseId,revision_id:revisionId,family_id:`grounded-${index+1}`,title:`Grounded question ${index+1}`,task_type:"grounded_qa" as const,domain:"generic",tags:["grounded"],language:"en",jurisdiction:null,as_of:null,difficulty:"routine" as const,severity:item.severity??"medium",split:"validation" as const,scenario:{mode:"single_turn" as const,required_capabilities:["text" as const],messages:[{role:"user" as const,content:item.question}],attachments:[],turn_plan:null,tool_fixture_set_id:null,termination:{kind:"final_answer" as const}},reference:{answerability:"answerable" as const,expected:item.expected,acceptable_alternatives:[],required_claims:[item.expected],prohibited_claims:[],prohibited_actions:[],source_refs:[{source_revision_id:input.source.revision_id,anchor:item.anchor}],rubric_revision_id:rubricRevisionId,graders:[{kind:"claims" as const,required:[item.expected],prohibited:[]}]},provenance:{method:"human_authored" as const,generator_revision:null,prompt_revision:null,evidence_level:"source_supported" as const,author_ids:[input.authorId],reviewer_ids:[],rights:"customer_owned" as const,created_at:createdAt},limits:{max_turns:1,max_output_tokens:500,max_tool_calls:0,timeout_ms:60000,repetitions:1},weight:1,extensions:{}});});
  return {rubric,cases};
}
