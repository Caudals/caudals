...
describe("operator console repository", () => {
  ...
  it("maps postgres rows into the console snapshot shape", async () => {
      ...
      if (sql.includes("work_items")) {
        return [
          ...
          },
          {
            ...
            record_type: "payment",
            id: "pt_01J2PAYMENT",
            title: "Payment USD 2500",
            state: "succeeded",
            detail: "pi_123",
            updated_at: "2026-05-10T13:17:50.000Z",
            severity: "info",
            next_action: "Review payment",
            field_values: {
              amountCents: 250000,
              currency: "USD",
          ...
          {
            module_key: "commercials",
            record_type: "revenue_share",
            id: "rs_01J2SHARE",
            title: "Revenue share USD 750",
            state: "approved",
            detail: "Pilot invoice less platform fee.",
            updated_at: "2026-05-10T13:17:40.000Z",
            severity: "warning",
            next_action: "Review revenue share",
            field_values: {
              shareRate: 0.3,
              grossCents: 250000,
              netCents: 75000,
          ...
          {
            module_key: "datasets",
            record_type: "dataset_partition",
            id: "dp_01J2PARTITION",
            title: "silver partition receipts/2026-05",
            state: "sealed",
            detail: "parquet / 1250000 records",
            updated_at: "2026-05-10T13:16:55.000Z",
            severity: "warning",
            next_action: "Review partition",
            field_values: {
              layer: "silver",
              objectUri: "s3://silver/receipts/2026-05/data.parquet",
          ...
          },
          {
            ...
            title: "Ana Lopez",
            state: "active",
            detail: "subject_matter_expert",
            updated_at: "2026-05-10T13:16:03.000Z",
            severity: "info",
            next_action: "Review reviewer access",
            field_values: {
              reviewerPool: "subject_matter_expert",
              skills: "document_ocr, spanish",
    ...
    expect(
      snapshot.workItems.commercials.find((item) => item.id === "pt_01J2PAYMENT")
    ).toMatchObject({
      recordType: "payment",
      fields: {
        amountCents: "250000",
    ...
    expect(
      snapshot.workItems.commercials.find((item) => item.id === "rs_01J2SHARE")
    ).toMatchObject({
    ...
    expect(
      snapshot.workItems.datasets.find((item) => item.id === "dp_01J2PARTITION")
    ).toMatchObject({
    ...
    expect(
      snapshot.workItems.datasets.find((item) => item.id === "ma_01J2MANIFEST")
    ).toMatchObject({
    ...
    expect(
      snapshot.workItems.labeling.find((item) => item.id === "rv_01J2REVIEWER")
    ).toMatchObject({
      recordType: "reviewer",
      fields: {
        reviewerPool: "subject_matter_expert",
    ...
    expect(workItemsSql).toContain("dataset_partition");
    expect(workItemsSql).toContain("manifest_artifact");
    ...
    expect(workItemsSql).toContain("reviewer");
    ...
    expect(workItemsSql).toContain("payment");
    expect(workItemsSql).toContain("revenue_share");
  });
