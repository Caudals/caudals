...
export function ImprovementDatasets({ workspaces }: { workspaces: Workspace[] }) {
  ...
  const releaseReady = useMemo(() => Boolean(detail?.items.length) && detail!.items.every((item) =>
  ...
  return <div className="p-stack">
    ...
    {tab === "build" && detail && <div className="p-stack">
      ...
      <Card title="4. Release checklist"><div className="p-stack"><Status tone={releaseReady ? "success" : "warn"}>{releaseReady ? "Every item has independent approval, permitted rights and approved redaction." : "Release requires independent approval, permitted rights and approved redaction for every item."}</Status><p>Release creates a frozen, signed JSONL artifact in private storage. Creating work or releasing data never changes the customer&apos;s system.</p><Action onClick={() => void release()} disabled={busy || !releaseReady}>Release signed dataset</Action></div></Card>
