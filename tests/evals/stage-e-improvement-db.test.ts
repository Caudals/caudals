...
(runtimeUrl && ownerUrl ? describe : describe.skip)("WP-15 improvement lineage PostgreSQL path", () => {
  ...
  const submission = randomUUID(); const assignment = randomUUID(); const guideline = randomUUID();
  ...
  async function createCandidate(input: { family: string; split: string; decision: string; reviewer: string }) {
    return withTenant({ orgId: orgA, actorId: operator }, async (db) => {
      ...
      const task = (await db.query(`INSERT INTO evals.improvement_task(org_id,batch_id,finding_id,expert_assignment_id,kind)
        VALUES($1,$2,$3,$4,'grounded_qa') RETURNING id`, [orgA, batch.id, finding, assignment])).rows[0];
